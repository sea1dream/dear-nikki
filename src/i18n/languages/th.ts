import Key from "../i18nKey";
import type { Translation } from "../translation";

export const th: Translation = {
    [Key.home]: "หน้าแรก",
    [Key.about]: "เกี่ยวกับเรา",
    [Key.archive]: "คลัง",
    [Key.link]: "ลิงก์",
    [Key.search]: "ค้นหา",

    [Key.tags]: "ป้ายกำกับ",
    [Key.categories]: "หมวดหมู่",
    [Key.siteStats]: "สถิติการเข้าชม",
    [Key.recentPosts]: "โพสต์ล่าสุด",

    [Key.comments]: "ความคิดเห็น",

    [Key.untitled]: "ไม่ได้ตั้งชื่อ",
    [Key.uncategorized]: "ไม่ได้จัดหมวดหมู่",
    [Key.noTags]: "ไม่มีป้ายกำกับ",

    [Key.wordCount]: "คำ",
    [Key.wordsCount]: "คำ",
    [Key.minuteCount]: "นาที",
    [Key.minutesCount]: "นาที",
    [Key.totalVisits]: "ยอดเข้าชมสะสม",
    [Key.todayVisits]: "ยอดเข้าชมวันนี้",
    [Key.todayVisitors]: "ผู้เข้าชมวันนี้",
    [Key.postCount]: "โพสต์",
    [Key.postsCount]: "โพสต์",

    [Key.themeColor]: "สีของธีม",

    [Key.lightMode]: "โหมดสว่าง",
    [Key.darkMode]: "โหมดมืด",
    [Key.systemMode]: "ตามระบบ",

    [Key.more]: "ดูเพิ่มเติม",

    [Key.author]: "ผู้เขียน",
    [Key.publishedAt]: "เผยแพร่เมื่อ",
    [Key.license]: "สัญญาอนุญาต",

    [Key.notFoundTitle]: "ไม่พบหน้าที่ต้องการ",
    [Key.notFoundDescription]: "ไม่พบหน้าที่คุณต้องการ อาจถูกย้ายหรือนำออกชั่วคราว",
    [Key.notFoundCta]: "กลับไปหน้าแรก",

    [Key.aboutMissingTitle]: "ไม่พบหน้า About",
    [Key.aboutMissingDescription]:
        "ไม่พบเนื้อหา {ENTRY} หากคุณเป็นผู้ดูแลระบบ โปรดเพิ่ม {FILE} หรืออัปเดตการตั้งค่าเนื้อหา",

    [Key.imageLoadError]: "ไม่สามารถโหลดรูปภาพได้",
    [Key.statsUnavailable]: "ไม่สามารถดึงสถิติได้ชั่วคราว",
};
