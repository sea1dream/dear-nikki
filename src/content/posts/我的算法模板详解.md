...
| `PrimeComb` | 质数模数下的阶乘、排列数与组合数 | 预处理摊还 $O(n)$；查询为 $O(1)$ 或按乘法步数与 Lucas 数位计算 |
...
整数几何使用的 `geometry::ExactInteger` 已经直接实现在模板内，不再依赖 Boost.Multiprecision。整个模板要求评测环境提供 C++23 与 GNU 扩展，旧版 GCC、MSVC 或较早的语言标准不能直接编译。模板顶部还会检查 `__cplusplus`，低于 C++23 时立即给出明确错误。
...
在质数模数 $p$ 下，对单个满足 $0 \le n < p$ 的数位有：

$$P(n,k)=\frac{n!}{(n-k)!}$$

$$C(n,k)=\frac{n!}{k!(n-k)!}$$

除法通过乘法逆元完成。`PrimeComb` 把“显式预处理”和“无需扩表的查询”分开：它只保存到 `prepared()` 为止的 `factorial[i] = i!` 与 `inverse_factorial[i] = (i!)^{-1}`，命中表的数位可在 $O(1)$ 内查询；没有命中时，`P` 使用直接乘法，`C` 使用乘法公式并对完整 `u64` 输入自动应用 Lucas 定理。`P` 和 `C` 都不会因为传入一个很大的 `n` 而自动申请同等规模的内存。
...
constexpr u64 huge_n = 1'000'000'000'000'000'000ULL;
auto permutation = comb.P(huge_n, 20);       // 不扩表，直接做少量乘法
auto combination = comb.C(huge_n, 20);       // 不扩表，按 Lucas 数位计算
```
...
`factorial(n)`、`inverse_factorial(n)` 和 `ensure(n)` 仍然属于阶乘表接口。模板在扩容时使用递推式求每个整数的逆元：
...
未缓存的排列在 $k\ge p$ 时必然跨过 $p$ 的倍数，直接返回 0；否则会在长度为 $k$ 的区间和它在 $[1,p-1]$ 中的补集之间选择较短的一侧，因此乘法次数不超过 $\min(k,p-1-k)$，补集分支还需要一次 $O(\log p)$ 的求逆。

组合数先把 $n$、$k$ 写成 $p$ 进制：$n=\sum n_i p^i$、$k=\sum k_i p^i$，再使用 Lucas 定理

$$C(n,k)\equiv\prod_i C(n_i,k_i)\pmod p.$$

只要某一位满足 $k_i>n_i$，结果就是 0。每个未缓存数位会选择普通乘法公式或靠近 $p-1$ 的补集公式，乘法次数约为 $\min(k_i,n_i-k_i,p-1-n_i)$，并需要一次快速求逆。它保证不按巨大 `n` 扩表，但不代表任意巨大 `k` 都能快速计算。
...
void solve() {
    ...
    constexpr u64 cache_limit = 2'000'000;
    ...
    PrimeComb comb(modulus);
    if (maximum_n < modulus && maximum_n <= cache_limit) {
        comb.ensure(maximum_n);
    }
...
当所有 `n` 都不超过 `cache_limit` 时，这段代码一次性预处理，之后每次查询为 $O(1)$，总复杂度为 $O(\max n+q)$。如果出现更大的 `n`，它不会冒险申请巨量内存，而是让 `C` 自动走 Lucas 与乘法公式；此时总时间取决于各个数位实际需要的乘法次数。

### 使用限制与内存

- 模数必须是质数；`P`、`C` 接受完整 `u64 n`，`k > n` 时返回 0，并且不会自动扩展阶乘表；
- `ensure(n)` 与 `inverse_factorial(n)` 要求 `n < modulus`；`factorial(n)` 在 `n >= modulus` 时按数学定义返回 0；
- 未缓存的 `P`、`C` 可能需要大量乘法，支持大范围不等于对所有参数都足够快；小质数且查询很多时可以预处理到 `modulus - 1`，大质数则应结合 `k` 和询问次数评估；
- 两张 `u64` 表的有效数据大约占 `16 × (n + 1)` 字节，预处理到 $10^6$ 时约为 16 MB；为摊还扩容而预留的容量可能略大；
- 非质数模数会抛出 `invalid_argument`，阶乘表接口越过适用范围会抛出 `out_of_range`；
...
计算几何最危险的地方通常不是公式本身，而是“看起来能装进 `i64`”的中间结果。两个完整范围的 `i64` 坐标相减可能需要 65 位，叉积还要继续相乘。模板现在内置 `geometry::ExactInteger`：它用四个 `u64` 保存 256 位绝对值并单独保存符号，因此移除了 Boost.Multiprecision 依赖。二维完整 `i64` 坐标的直接谓词最坏约需 130 位，三维四点定向体积最坏少于 195 位，因此都有充足余量：
...
auto twice_area = geometry::cross(a, b, c);  // ExactInteger，精确值为 12。
int turn = geometry::orientation(a, b, c);   // 1，表示逆时针。
```

`ExactInteger` 支持带符号加减乘除、取模、比较、十进制字符串和流输出；除数为 0 会抛出 `invalid_argument`，结果超出定长范围则抛出 `overflow_error`。它是为几何谓词设计的定长整数，不是任意精度大整数。

`Calculation<T>` 对普通整数坐标与 `ExactInteger` 坐标选择 `ExactInteger`，对浮点坐标选择 `long double`。因此普通整数版 `dot`、`cross`、`norm_squared`、`distance_squared` 和 `orientation` 都不依赖 EPS，并覆盖完整 `i64` 输入。直接谓词有上述位宽保证，但对大量多边形项求和或手动连续运算 `ExactInteger` 时仍可能超过 256 位，此时模板会明确抛出异常而不是静默回绕。`Point2<i64>` 自身的加减乘仍要把结果存回 `i64`，若坐标越界也会抛出 `overflow_error`；整数点做除法还会遵循整数除法语义，需要非整数结果时应先转成 `Point`。
...
三维部分延续二维的类型策略：`Point3D` 是 `Point3<long double>`，`IPoint3` 是 `Point3<i64>`。整数 `dot`、`cross`、`triple`、距离平方和四点定向体积都会先提升到 `ExactInteger`；三维叉积返回 `Point3<ExactInteger>`，不会把分量窄化回原坐标类型。
...
6. `PrimeComb` 的模数必须是质数；只有阶乘表接口要求 `n < modulus`。`P`、`C` 支持完整 `u64 n` 且不会自动扩表，但未缓存数位可能需要大量乘法。
...
24. 几何模块不再依赖 Boost，但内置的 `ExactInteger` 是定长 256 位绝对值而非任意精度整数；超出范围的连续运算会抛出 `overflow_error`。
...
27. 完整 `i64` 坐标的直接叉积、点积、距离平方与三维定向体积会安全提升到 `ExactInteger`，但大量项累加仍可能超过 256 位；点坐标加减乘还要存回原类型，越界会抛异常，整数点除法也会发生截断。
