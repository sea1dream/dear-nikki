...
updated: 2026-08-24
description: 逐段拆解我的算法竞赛模板，包括随机盐哈希、PBDS、扩展欧几里得、Möbius 反演、Stirling 数，以及普通与位掩码卷积，并配上可运行示例。
tags: [C++, 算法竞赛, 模板, 数论, 位运算, FFT, NTT, FWT, 多项式, Möbius反演, 组合数学]
category: 算法
draft: false
---

这份模板不是把常见代码无脑堆在一起，而是尽量把竞赛中容易写错的边界统一封装起来：哈希表怎样缓解构造碰撞、重复元素如何做顺序统计、`clz(0)` 和 `ctz(0)` 怎么处理、线性同余与非互质模数怎样合并、Möbius 与子集反演怎样安全还原，以及不同模数、精度和下标代数下该选择哪种卷积。
...
18. FWT/FMT 与子集卷积的两组输入必须等长，非空长度必须是二次幂；它们不会像普通卷积一样自动补齐。
19. XOR FWT 的逆变换在长度大于 1 时要求奇数模数，因为需要计算 $2^k$ 的模逆。
20. 标准子集卷积的空间复杂度是 $O(k2^k)$，当前实现在 $k=20$ 时约占 350 MiB。
21. 模板是工具箱，不是每题都必须完整提交；对编译时间或代码长度敏感时，只保留用到的模块。
