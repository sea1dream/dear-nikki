...
description: 逐段拆解我的算法竞赛模板，包括 PBDS、位运算、溢出检测、调试工具、Miller–Rabin 与组合数，并配上可运行示例。
tags: [C++, 算法竞赛, 模板, 数论, 位运算]
category: 算法
draft: false
---

这份模板不是把常见代码无脑堆在一起，而是尽量把竞赛中容易写错的边界统一封装起来：重复元素如何做顺序统计、`clz(0)` 和 `ctz(0)` 怎么处理、64 位乘法怎样避免溢出、如何判断完整 `uint64_t` 范围内的素数，以及组合数预处理如何按需扩容。
...
10. 模板是工具箱，不是每题都必须完整提交；对编译时间或代码长度敏感时，只保留用到的模块。
