...
updated: 2026-08-25
description: 逐段拆解我的算法竞赛模板，包括数论、组合数学、矩阵、二维与三维计算几何，以及普通和位掩码卷积，并配上可运行示例。
tags: [C++, 算法竞赛, 模板, 数论, 位运算, FFT, NTT, FWT, 多项式, Möbius反演, 组合数学, 矩阵, 高斯消元, 矩阵树定理, 计算几何, 凸包, 半平面交]
category: 算法
draft: false
---
...
因此它要求评测环境同时提供 GNU 扩展与 Boost 头文件，不保证能直接通过 MSVC 或严格 ISO C++17 编译。若题目明确使用 Clang、MSVC 或其他工具链，需要替换这些扩展。
...
`Calculation<T>` 对整数坐标选择 `ExactInteger`，对浮点坐标选择 `long double`。因此整数版 `dot`、`cross`、`norm_squared`、`distance_squared` 和 `orientation` 都是精确谓词，覆盖完整 `i64` 输入，不依赖 EPS。`Point2<i64>` 自身的加减乘仍要把结果存回 `i64`，若坐标越界会抛出 `overflow_error`；整数点做除法还会遵循整数除法语义，需要非整数结果时应先转成 `Point`。

浮点版统一使用 `long double`，默认误差为 `geometry::DEFAULT_EPS = 1e-12L`。`almost_equal`、`sign` 和 `orientation` 接受自定义 `epsilon`，但默认值只是通用起点，不可能适配所有坐标尺度。坐标达到 $10^{12}$ 或经过大量运算时，应根据题目误差和数值尺度调整。
...
23. 几何模块依赖 Boost.Multiprecision；评测环境缺少 Boost 头文件时，整份模板会在编译阶段失败。
24. `DEFAULT_EPS` 不是适用于所有题目的常数；整数几何应优先使用 `IPoint`/`IPoint3` 的精确谓词，浮点几何则按坐标尺度和题目误差调整 EPS。
25. 排序比较器不能使用 EPS，否则可能破坏严格弱序；浮点凸包的排序去重是精确比较，输入中近似重复的点应按题意预处理。
26. 整数点的叉积、点积和距离平方会提升到 `int256_t`，但点坐标加减乘仍要存回原类型，越界会抛异常；整数点除法还会发生截断。
27. 整数线段可以精确判断相交类型，但一般交点是有理数；需要实际交点时应转成浮点线段后调用 `intersect_segments`。
28. `convex_hull` 与闵可夫斯基和的输出不重复首点；`minkowski_sum_convex_ccw` 只接收严格逆时针凸包，任意点集应调用会先求凸包的 `minkowski_sum`。
29. 半平面保留有向边左侧；方向写反会得到补集。一般入口还要检查 `Empty`、`Unbounded`、`BoundedPolygon` 与 `BoundedDegenerate`，不能只读取 `polygon`。
30. 三维 `Line3` 与 `Plane3` 要保持单位方向/法向量，优先使用返回 `optional` 的工厂函数，不要随意聚合初始化未经归一化的数据。
31. FWT/FMT 与子集卷积的两组输入必须等长，非空长度必须是二次幂；它们不会像普通卷积一样自动补齐。
32. XOR FWT 的逆变换在长度大于 1 时要求奇数模数，因为需要计算 $2^k$ 的模逆。
33. 标准子集卷积的空间复杂度是 $O(k2^k)$，当前实现在 $k=20$ 时约占 350 MiB。
34. 模板是工具箱，不是每题都必须完整提交；对编译时间或代码长度敏感时，只保留用到的模块。
