...
updated: 2026-08-24
description: 逐段拆解我的算法竞赛模板，包括随机盐哈希、PBDS、位运算、溢出检测、Miller–Rabin、组合数与四种多项式卷积，并配上可运行示例。
tags: [C++, 算法竞赛, 模板, 数论, 位运算, FFT, NTT, 多项式]
category: 算法
draft: false
---

这份模板不是把常见代码无脑堆在一起，而是尽量把竞赛中容易写错的边界统一封装起来：哈希表怎样缓解构造碰撞、重复元素如何做顺序统计、`clz(0)` 和 `ctz(0)` 怎么处理、64 位乘法怎样避免溢出、如何判断完整 `uint64_t` 范围内的素数、组合数预处理如何按需扩容，以及不同模数和精度要求下该选择哪种多项式卷积。
...
15. 模板是工具箱，不是每题都必须完整提交；对编译时间或代码长度敏感时，只保留用到的模块。
